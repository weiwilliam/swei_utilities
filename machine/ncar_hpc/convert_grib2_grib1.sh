...
while [ $cdate -le $edate ]; do
   ...
   do
     ...
     tmpfile=$wrkdir/tmpgrib2
     [[ -s $tmpfile ]] && rm $tmpfile
     outfile=$cdate_outputpath/pgbf${o_fhrstr}.${dump}.${cdate}.grib2
      
$WGRIB2 -ncpu 1 $infile -set_grib_type same -new_grid_winds earth -new_grid_interpolation bilinear -if ':(CSNOW|CRAIN|CFRZR|CICEP|ICSEV):' -new_grid_interpolation neighbor -fi -set_bitmap 1 -set_grib_max_bits 16 -if ':(APCP|ACPCP|PRATE|CPRAT):' -set_grib_max_bits 25 -fi -if ':(APCP|ACPCP|PRATE|CPRAT|DZDT):' -new_grid_interpolation budget -fi -new_grid latlon 0:360:1.0 90:181:-1.0 $tmpfile

$WGRIB2 -ncpu 1 $tmpfile -not_if :RH: -grib $outfile -if :RH: -rpn '10:*:0.5:+:floor:1000:min:10:/' -set_grib_type same -set_scaling -1 0 -grib_out $outfile
